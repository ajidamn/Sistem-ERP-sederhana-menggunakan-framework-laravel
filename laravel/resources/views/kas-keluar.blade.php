                        ...
                        <div class="form-group">
                            <div class="row">
                                ...
                                <div class="col-lg-4">
                                    ...
                                    <select id="tmb-inv" class="form-control select2"></select>
                                </div>
                                <div class="col-lg-4">
                                    <label> Kas</label>
...
<script>
  ...
  //TAMBAH DATA
    $(document).on('click','#tambahdata',function(){
        $('#tmb-tgl').prop('disabled',false);$('#tmb-kredit').prop('disabled',false);$('#tmb-keterangan').prop('disabled',false);$('#tmb-debit').prop('disabled',false);$('#tmb-inv').prop('disabled',false);
        $('#btn-add-barang').hide();$('#tambah-barang').hide();$('#edit-barang').hide();$('#hapus-barang').hide();
        $('#tmb-debit').select2({
          placeholder : 'Pilih Keperluan',
          ajax  :{
            url : '{!! url("dropdown-uangkeluar") !!}',
            dataType: 'json',
    ...
    $('#tmb-kredit').on('change',function(){
        var data = $(this).val();
        if(data == 40) {
           $('#tmb-inv').prop('disabled',false);$('#tmb-vat').prop('disabled',true);
           $('#tmb-inv').select2({
            placeholder : 'Pilih Invoice',
            ajax  :{
                url : '{!! url("dropdown-inv") !!}',
                dataType: 'json',
                processResults: function (data) {
                    return {
                        results: $.map(data, function (item) {
            ...
            $('#tmb-inv').val(null).trigger('change');
            $('#tmb-inv').prop('disabled',true); $('#tmb-vat').prop('disabled',false);
    ...
    //Tambah Barang
        ...
        $('#tambah-nama-barang').on('change',function(){
            ...
            var pemasukkan = $('#tmb-kredit').val();
            if (pemasukkan == 40 ){
                var invoice = $('#tmb-inv').val();
                $.ajax({
                url     : '{!! url("databarang-detailinv/'+invoice+'")!!}',
                type    : 'get',
                data    : {
                    barang  : barang,
                },
                success : function(response){
                    console.log(response);
                    if(response.success == true) {
                    $('#tambah-invoice-barang').val(invoice);
